...
prepare_apps(){
  ...
  for i in "${!APPS[@]}"; do
    ...
    dir="$(prepare_app_workdir "$app" "$url" "$root")"

    SOURCE_ID="$app" PATCH_ROOT="$dir/patches" run_user_build_script "$dir" "$script"
  done
}
...
build_inside_container(){
  ...
  prepare_apps /work/work "${BUILD_SCRIPT:-}"
...
deb_install_block(){
  ...
  local repo_file="$3"
  local packages="$4"
  local key_url="https://$owner.github.io/$repo/GPG-KEY-repo"
  local sources_url="https://$owner.github.io/$repo/$repo_file"
  local keyring="/usr/share/keyrings/$repo.gpg"
...
cmd_update_readme(){
  ...
  local generated packages row repo_id repo_file repo_path label family arch tool url app_list
  ...
  if [[ "$package_type" == flatpak ]]; then
    ...
    while IFS=$'\t' read -r repo_id repo_file repo_path label; do
      ...
      tool="$(target_install_tool "$package_type" "$family" "$arch")"
